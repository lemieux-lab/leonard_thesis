...
to_plot = DataFrame(Dict("emb_size"=>embsizes, "speed"=>avgspeeds))
...
fig = Figure();
ax = Axis(fig[1,1], title = "Average speed of training by number of nodes in the gene embedding layer", xlabel = "number of nodes in gene embedding layer", ylabel = "average speed (iteration/second)", xticks = (collect(1:length(posit)),string.(emb_sizes_sorted)) );
