...
dim_redux_size = 1000
input_type = "FE"
nsteps_dim_redux = 40_000
printstep_FE = 100 
nsteps_CPHDNN = 10_000 
printstep_CPHDNN = 1000 
...
generate_params(X_data, emb_size) = return Dict( 
    ...
    ## optim infos 
    "lr" => 1e-3, "l2" => 1e-8,"nsteps" => nsteps_dim_redux, "nsteps_inference" => Int(floor(nsteps_dim_redux * 0.1)), "nsamples_batchsize" => 1,
    ## model infos
...
cphdnn_params_dict(X_data) = Dict(
    ...
    "cph_hl_size"=> 64,
    "lr"=>1e-5, "l2"=>1e-3, "nsteps"=>nsteps_CPHDNN,
...
# for fold in folds
    ...
    # train FE 
FE_params_dict = generate_params(BRCA_data, dim_redux_size)
trained_FE,  tr_epochs , tr_loss, tr_cor =  generate_patient_embedding(BRCA_data, samples, genes, FE_params_dict, labs)
# infer FE
...
# end 
outf = h5open("CPHDNN_RES/$(FE_params_dict["modelid"])_FE_$(FE_params_dict["dim_redux_size"])D_BRCA.h5", "w")
outf["data"] = Matrix(cpu(trained_FE[1][1].weight)')
outf["survt"] = survt 
outf["surve"] = surve 
outf["samples"] = samples 
close(outf)

folds = split_train_test(Matrix(cpu(trained_FE[1][1].weight)'), nfolds = 5) # split 80-20
# init performance loggers
test_c_ind_by_fold, tst_scores_by_fold, tst_t_by_fold, tst_e_by_fold = [], [], [], []
# loop
for fold in folds
    train_ids, train_data, test_ids, test_data = fold["train_ids"], fold["train_x"], fold["test_ids"], fold["test_x"]
    train_t, train_e, test_t, test_e  = survt[train_ids],  surve[train_ids], survt[test_ids], surve[test_ids]
    tst_c_ind, tst_scores, y_t_test, y_e_test = CPHDNN_eval(gpu(train_data), gpu(train_t), gpu(train_e), gpu(test_data), gpu(test_t), gpu(test_e))
    println(tst_c_ind)
    push!(test_c_ind_by_fold, tst_c_ind)
    push!(tst_scores_by_fold, cpu(vec(tst_scores)))
    push!(tst_t_by_fold, cpu(vec(y_t_test)))
...
folds = split_train_test(BRCA_data, nfolds = 5) # split 80-20
# init performance loggers
test_c_ind_by_fold, tst_scores_by_fold, tst_t_by_fold, tst_e_by_fold = [], [], [], []
# loop
for fold in folds
